...
​Manajemen Biji Kopi (Beans Manajemen): Fitur CRUD (Create, Read, Update, Delete) untuk mencatat data biji kopi (Nama Roaster, Asal/ Origin, Proses, Tingkat Sangrai, dan Tanggal Sangrai).
​Jurnal Seduhan (Brew Journal): Fitur mencatat parameter seduhan yang terhubung dengan biji kopi tertentu (Metode/Alat seduh, Grind Size, Berat Kopi, Berat Air/ Yield, Waktu Seduh, dan Suhu).
​Catatan Evaluasi (Tasting Notes & Rating): Fitur untuk memberikan penilaian (bintang 1-5) dan catatan rasa tekstual pada setiap seduhan.
​Penyimpanan Lokal (Local Storage): Data disimpan secara lokal di dalam perangkat pengguna agar aplikasi dapat digunakan secara offline dengan cepat.
​Fitur yang Tidak Dikerjakan (Out of Scope)
Mengingat batasan waktu 12 pertemuan, beberapa fitur kompleks dari referensi asli (Beanconqueror) tidak akan dikerjakan pada fase ini:
